...
%       BinWidth.
%
%       UsePCA:        (Optional) False (default) or True. If True, applies
%       PCA dimensionality reduction to each population before CCA.
%
%       NumPCs:        (Optional) Number of principal components to keep
%       when UsePCA is True. Default is 20. Can be a scalar (same for both
%       areas) or a 2-element vector [numPCs_area1, numPCs_area2].
% 
...
%
%       PCAInfo:         (Optional) structure containing PCA information if
%       UsePCA is True. Contains coefficients, explained variance, etc.
...
C_CV_NUM_FOLDS = argIn.C_CV_NUM_FOLDS;
...
if ~isfield(argIn, 'UsePCA')
    argIn.UsePCA = false;
end
if ~isfield(argIn, 'NumPCs')
    argIn.NumPCs = 20;
end

% Handle NumPCs as scalar or 2-element vector
if isscalar(argIn.NumPCs)
    numPCs = [argIn.NumPCs, argIn.NumPCs];
else
    numPCs = argIn.NumPCs;
end
...
if argIn.UsePCA
    ...
    [resids, argOut.PCAInfo] = ApplyPCA(resids, numPCs, expCond);
    
    % Update numUnits to reflect reduced dimensionality
    numUnits(pop1) = size(resids{pop1}, 1);
    numUnits(pop2) = size(resids{pop2}, 1);
    
    fprintf('  Area 1: %d neurons -> %d PCs (%.1f%% variance explained)\n', ...
        argOut.PCAInfo{pop1}.numOriginalUnits, numUnits(pop1), ...
        sum(argOut.PCAInfo{pop1}.explained(1:numUnits(pop1))));
    fprintf('  Area 2: %d neurons -> %d PCs (%.1f%% variance explained)\n', ...
        argOut.PCAInfo{pop2}.numOriginalUnits, numUnits(pop2), ...
        sum(argOut.PCAInfo{pop2}.explained(1:numUnits(pop2))));
    
    % Check sample-to-dimension ratio
    N_samples = argIn.WindowLength/(argIn.BinWidth/C_INTERNAL_BIN_WIDTH)*numTrials;
    ratio1 = N_samples / numUnits(pop1);
    ratio2 = N_samples / numUnits(pop2);
    fprintf('  Sample/dimension ratios: Area1=%.1f, Area2=%.1f', ratio1, ratio2);
    if min(ratio1, ratio2) < 20
        warning(' ⚠️ Ratio < 20! Consider using fewer PCs or longer window.');
    else
        fprintf(' ✓\n');
    end
end
...
% =========================================================================
% NEW FUNCTION: Apply PCA
% =========================================================================
function [resids_pca, pca_info] = ApplyPCA(resids, numPCs, expCond)
% ApplyPCA - Apply PCA dimensionality reduction to residuals
%
% INPUTS:
%   resids  - cell array {area1, area2} with residuals (neurons × time × trials)
%   numPCs  - 2-element vector [numPCs_area1, numPCs_area2]
%   expCond - trial condition labels
%
% OUTPUTS:
%   resids_pca - cell array with PCA-transformed data (PCs × time × trials)
%   pca_info   - cell array with PCA information for each area

pop1 = 1;
pop2 = 2;

resids_pca = cell(size(resids));
pca_info = cell(size(resids));

for area = [pop1, pop2]
    
    % Get data dimensions
    [N, T, K] = size(resids{area});
    
    % Store original number of units
    pca_info{area}.numOriginalUnits = N;
    
    % Reshape: (neurons × time × trials) → (neurons × samples)
    data = reshape(resids{area}, N, T*K);
    
    % Apply PCA (transpose so samples are rows)
    [coeff, score, ~, ~, explained, mu] = pca(data');
    % coeff: (neurons × PCs) - PC loadings
    % score: (samples × PCs) - PC scores
    % explained: variance explained by each PC
    % mu: mean of each neuron
    
    % Determine number of PCs to keep
    nPCs = min(numPCs(area), size(score, 2));
    
    % Keep top PCs and reshape back
    score_reduced = score(:, 1:nPCs);  % (samples × nPCs)
    resids_pca{area} = reshape(score_reduced', nPCs, T, K);
    
    % Store PCA information
    pca_info{area}.coeff = coeff(:, 1:nPCs);
    pca_info{area}.explained = explained;
    pca_info{area}.mu = mu;
    pca_info{area}.numPCs = nPCs;
    pca_info{area}.totalVarianceExplained = sum(explained(1:nPCs));
