function plotspkrate_correct(dataStruct, timeInfo, var, eventInfo, session_num, iUnit)
...
for correct_val = [1, 0]
    ...
    sgtitle(sprintf('unit %d, %s, depth %d (%s Trials)', unit_num, Info_name{1}, ...
        dataStruct(session_num).data.dots3DMP.unit.depth(iUnit), trial_type));
    ...
    for i = 1:length(alignEvent)
        ...
        for cond = 1:length(condition_indices)
            psth_data = calculatePSTH(psth, headingInd, mod, coh, del, correct, correct_val, ...
                condition_indices{cond}, var, class_info, num_classes, yAxis, dataStruct, session_num, Info_name);
    ...
    if y_lim >= 1.5
        fig = gcf;
        [filepath, name, ext] = fileparts(eventInfo.output_pdf{var});
        output_file = fullfile(filepath, [name '_correct' ext]);
...
function psth_data = calculatePSTH(psth, headingInd, mod, coh, del, correct, correct_val, condition_func, ...
...
switch var
    case 6
        for log = 1:7
            idx = condition_func(mod, coh) & headingInd==log & del'==0 & correct==correct_val & ...
    ...
    case 7
        ...
        for log = 1:4
            ...
            idx = condition_func(mod, coh) & ismember(headingInd, [3,4,5]) & del'==0 & correct==correct_val & ...
    ...
    otherwise
        ...
        for log = 1:2
            idx = condition_func(mod, coh) & logit_ind==(log-1) & del==0 & correct==correct_val & ...
                ~cellfun(@(x) any(isnan(x)), psth)';
            if any(idx)
                data = cell2mat(psth(idx));
